    ...
    ***************************************************************************** */
    ...
    function doWatch(source, cb, _a) {
        ...
        if (isReactive(source)) { //判断source的数据类型，如果是响应式对象就将getter函数返回它
            ...
            getter = function () { }; //否则就将getter返回值赋值为空对象
        }
        if (cb && deep) { //如果有回调函数，并且配置对象中开启了深度监听
            var baseGetter_1 = getter;
            getter = function () { return traverse(baseGetter_1()); }; //防止深度监听的对象中存在递归调用
        }
        var oldValue = {}; //旧值
        var job = function () {
            if (cb) {
                var newValue = effect.run(); //如果有回调函数，就执行得到新值
        ...
        // 调度器
        var scheduler = function () { return queuePreFlushCb(job); }; //调度器的作用是改变函数的执行顺序 
    ...
    function traverse(value, seen) {
        ...
        seen = seen || new Set();
        seen.add(value);
        for (var key in value) { //便利对象，使得对象属性被依赖收集
            traverse(value[key], seen); //递归调用自身，使用seen set集合的作用是防止对象中递归引用造成死循环
        }
        return value;
    }
